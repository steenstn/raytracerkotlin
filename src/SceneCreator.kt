...
class SceneCreator {
    companion object {
        ...
        fun molecule() : Scene {
            ...
            var position = Vector()
            for (i in 0..200) {
                val newDir = Vector.random().normalize()*1.1
                position+=newDir
                val color = if(Random.nextDouble() > 0.7) Vector(1.0,0.2,0.33) else Vector(0.65,1.2,0.64)
                spheres.add(Sphere(position.x,position.y,position.z,1.0, Material.diffuse(color.x, color.y, color.z)))
            }
